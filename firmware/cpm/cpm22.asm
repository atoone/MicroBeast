...
;*                by Clark A. Calkins
;*
;*      12/6/2017 Edits to make TASM happy. J.C.Lang
;**************************************************************
	.PAGE
	.TITLE "CP/M Ver 2.2 CCP"
	.EJECT
;**************************************************************
; CP/M Version 2.2 Command Processor
;**************************************************************
;
...
TDRIVE	.EQU	4		;current drive name and user number. 0=A: 1=B:... (was D:)
...
;
INBUFF:
	.BYTE	127		;length of input buffer.
	.BYTE	0		;current length of contents.
	.BYTE	"Copyright"
	.BYTE	" 1979 "
	.BYTE	28h,63h,29h
	.BYTE	" by Digital Research      "
	...
	.BYTE	0,0,0,0,0,0,0
INPOINT:
	.WORD	INBUFF+2	;input line pointer
NAMEPNT:
	.WORD	0		;pointer used for error message.
...
;
OPENFCB:
	XOR	A		;clear the record number byte
	LD	(FCB+32),A	;at fcb+32
	LD	DE,FCB
	JP	OPEN
;
;-------------------------------------------------------------
...
SETCDRV:
	...
	ADD	A,A
	ADD	A,A
	LD	HL,CDRIVE	;add in the current drive number.
	OR	(HL)
...
;
UPPER:
	CP	'a'		;letters in the range of 'a' to 'z'?
	RET	C
	CP	'{'
	RET	NC
	AND	5FH		;convert to lower case if found.
	RET	
;
;-------------------------------------------------------------
...
;
;-------------------------------------------------------------
;   Check the CCP and BDOS serial numbers
;  They must be the same or we halt....
;  (anti piracy protection?) (it didn't work)
;
VERIFY:
	LD	DE,PATTRN1	;CCP serial number bytes.
	LD	HL,PATTRN2	;BDOS serial number bytes
	LD	B,6		;6 bytes each.
VERIFY1:
	LD	A,(DE)
	CP	(HL)
...
CONVERT:
	...
	CP	':'
	JP	Z,CONVRT2
	DEC	DE		;move pointer back to the start of the line.
CONVRT1:
	LD	A,(CDRIVE)
	LD	(HL),A
	JP	CONVRT3
...
;-------------------------------------------------------------
;   CP/M command table.
;   Note commands can be either 3 or 4 characters long.
;
...
;
;-------------------------------------------------------------
; PATTRN1 is the CCP serial number
; PATTRN2 is the BDOS serial number
; They must match
;
PATTRN1:
	.BYTE	0,22,0,0,0,0	;(* serial number bytes *).
;
;-------------------------------------------------------------
...
;
CMDADR:
	.WORD	DIRECT,ERASE,TYPE,SAVE
	.WORD	RENAME,USER,UNKNOWN
;
;-------------------------------------------------------------
;   Halt the system. This is done if the CCP
; and BDOS serial numbers don't match
;
HALT:
...
; one reference to this near UNKWN2:.
;
MOVE3:
	LD	B,3
;
...
;
UNKNOWN:
	CALL	VERIFY		;check for valid system.
...
BADLOAD:
	.BYTE	"Bad load",0
COMFILE:
	.BYTE	"COM"		;command file extension.
;
...
;
BATCH:
	.BYTE	0	;batch mode flag (0=not active).
BATCHFCB:
	.BYTE	0
	.BYTE	"$$$     SUB"
	.BYTE	0,0,0,0,0,0,0,0
...
FCB:
	...
	.BYTE	0,0,0,0,0
RTNCODE:
	.BYTE	0	;status returned from bdos call.
CDRIVE:
	.BYTE	0	;currently active drive.
CHGDRV:
	.BYTE	0	;change in drives flag (0=no change).
NBYTES:
	.WORD	0	;byte counter used by TYPE.
;
;-------------------------------------------------------------
;   Room for expansion?
;
...
;
	.EJECT
	.ORG CBASE+800h
	.TITLE " CP/M Ver 2.2 BDOS"
	.EJECT
;**************************************************************
; CP/M Version 2.2 Basic Disk Operating System
;**************************************************************
;
PATTRN2:
	.BYTE	0,22,0,0,0,0	;BDOS serial number bytes.
;
FBASE:
	JP	FBASE1		;BDOS entry point
;
;-------------------------------------------------------------
;   Bdos error table.
;
BADSCTR:
	.WORD	ERROR1		;bad sector on read or write.
BADSLCT:
	.WORD	ERROR2		;bad disk select.
RODISK:
	.WORD	ERROR3		;disk is read only.
ROFILE:
	.WORD	ERROR4		;file is read only.
;
	.eject
;
;-------------------------------------------------------------
...
;
FUNCTNS:
	.WORD	WBOOT,GETCON,OUTCON,GETRDR,PUNCH,LIST,DIRCIO,GETIOB
	.WORD	SETIOB,PRTSTR,RDBUFF,GETCSTS,GETVER,RSTDSK,SETDSK,OPENFIL
	.WORD	CLOSEFIL,GETFST,GETNXT,DELFILE,READSEQ,WRTSEQ,FCREATE
	.WORD	RENFILE,GETLOG,GETCRNT,PUTDMA,GETALOC,WRTPRTD,GETROV,SETATTR
	.WORD	GETPARM,GETUSER,RDRANDOM,WTRANDOM,FILESIZE,SETRAN,LOGOFF,RTN
	.WORD	RTN,WTSPECL
;
;-------------------------------------------------------------
...
RDBUF7:
	...
	JP	RDBUF7
RDBUF8:
	CP	CNTRLU		;cntrol-u (cancel line)?
	JP	NZ,RDBUF9
...
;
GETRDR:
	CALL	READER		;get a character from reader
...
;-------------------------------------------------------------
OUTFLAG:
	.BYTE	0		;output flag (non zero means no output).
STARTING:
	.BYTE	2		;starting position for cursor.
CURPOS:
	.BYTE	0		;cursor position (0=start of line).
PRTFLAG:
	.BYTE	0		;printer flag (control-p toggle). List if non zero.
CHARBUF:
	.BYTE	0		;single input character buffer.
;
;-------------------------------------------------------------
;   Stack area for BDOS calls.
;
USRSTACK:
	.WORD	0		;save users stack pointer here.
;
	.BYTE	0,0,0,0,0,0,0,0
	.BYTE	0,0,0,0,0,0,0,0
	.BYTE	0,0,0,0,0,0,0,0
...
;-------------------------------------------------------------
USERNO:
	.BYTE	0		;current user number.
ACTIVE:
	.BYTE	0		;currently active drive.
PARAMS:
	.WORD	0		;save (DE) parameters here on entry.
STATUS:
	.WORD	0		;status returned from bdos function.
;
;-------------------------------------------------------------
...
;	XOR	A		;select drive 'A'.
	LD	A,SYSDSK	;select system drive
	LD	(ACTIVE),A
...
;
;-------------------------------------------------------------
;   Function to write the next sequential record.
;
WRTSEQ:
...
;
EMPTYFCB:
	.BYTE	0E5H		;empty directory segment indicator.
WRTPRT:
	.WORD	0		;write protect status for all 16 drives.
LOGIN:
	.WORD	0		;drive active word (1 bit per drive).
USERDMA:
	.WORD	080H		;user's dma address (defaults to 80h).
;
;-------------------------------------------------------------
;   Scratch areas from parameter block.
;
SCRATCH1:
	.WORD	0		;relative position within dir segment for file (0-3).
SCRATCH2:
	.WORD	0		;last selected track number.
SCRATCH3:
	.WORD	0		;last selected sector number.
;
;-------------------------------------------------------------
;   Disk storage areas from parameter block.
;
DIRBUF:
	.WORD	0		;address of directory buffer to use.
DISKPB:
	.WORD	0		;contains address of disk parameter block.
CHKVECT:
	.WORD	0		;address of check vector.
ALOCVECT:
	.WORD	0		;address of allocation vector (bit map).
;
;-------------------------------------------------------------
;   Parameter block returned from the bios.
;
SECTORS:
	.WORD	0		;sectors per track from bios.
BLKSHFT:
	.BYTE	0		;block shift.
BLKMASK:
	.BYTE	0		;block mask.
EXTMASK:
	.BYTE	0		;extent mask.
DSKSIZE:
	.WORD	0		;disk size from bios (number of blocks-1).
DIRSIZE:
	.WORD	0		;directory size.
ALLOC0:
	.WORD	0		;storage for first bytes of bit map (dir space used).
ALLOC1:
	.WORD	0
OFFSET:
	.WORD	0		;first usable track number.
XLATE:
	.WORD	0		;sector translation table address.
;
;-------------------------------------------------------------
;
CLOSEFLG:
	.BYTE	0		;close flag (=0ffh is extent written ok).
RDWRTFLG:
	.BYTE	0		;read/write flag (0ffh=read, 0=write).
FNDSTAT:
	.BYTE	0		;filename found status (0=found first entry).
MODE:
	.BYTE	0		;I/o mode select (0=random, 1=sequential, 2=special random).
EPARAM:
	.BYTE	0		;storage for register (E) on entry to bdos.
RELBLOCK:
	.BYTE	0		;relative position within fcb of block number written.
COUNTER:
	.BYTE	0		;byte counter for directory name searches.
SAVEFCB:
	.WORD	0,0		;save space for address of fcb (for directory searches).
BIGDISK:
	.BYTE	0		;if =0 then disk is > 256 blocks long.
AUTO:
	.BYTE	0		;if non-zero, then auto select activated.
OLDDRV:
	.BYTE	0		;on auto select, storage for previous drive.
AUTOFLAG:
	.BYTE	0		;if non-zero, then auto select changed drives.
SAVNXT:
	.BYTE	0		;storage for next record number to access.
SAVEXT:
	.BYTE	0		;storage for extent number of file.
SAVNREC:
	.WORD	0		;storage for number of records in file.
BLKNMBR:
	.WORD	0		;block number (physical sector) used within a file or logical sect
LOGSECT:
	.WORD	0		;starting logical (128 byte) sector of block (physical sector).
FCBPOS:
	.BYTE	0		;relative position within buffer for fcb of file of interest.
FILEPOS:
	.WORD	0		;files position within directory (0 to max entries -1).
;
;-------------------------------------------------------------
...
;**************************************************************
;
;labels for the entry points into the BIOS
;
BOOT:	.BLOCK	3
WBOOT:	.BLOCK	3
CONST:	.BLOCK	3
